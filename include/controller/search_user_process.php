...
include('../../include/layout/db_connection.php');
	...
	{ 
		...
		$result = search_user($id, $username);
		
		//create user messages
		if($result===1)
		{
			//if product is successfully added, create a success message to display on the products page
			$_SESSION['success'] = 'User successfully searched.';
			$_SESSION['search-id']=$id;
			$_SESSION['search-name']=$username;
			foreach($result as $row)
			$id= $row['user_id'];
			//redirect to products.php
			redirect('..\..\public\pages\user-mang.php#update?userID='.$id);
		}
		else
		{
